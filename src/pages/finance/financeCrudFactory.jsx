...
export function createFinanceListPage(config) {
  return function FinanceListPage() {
    ...
    return (
      <div className="finance-page finance-crud-page">
        <div className="finance-page__header finance-page__header--with-actions">
          <div>
            <h2>{config.pageTitle}</h2>
            <p>Manage {config.labelPlural.toLowerCase()} with live database records.</p>
          </div>
          <div className="finance-actions">
            <button type="button" className="finance-btn" onClick={() => navigate(config.newRoute)}>
              {config.addLabel}
            </button>
          </div>
        </div>

        <div className="finance-cards finance-cards--compact">
        ...
        <div className="finance-panel">
          <div className="finance-panel__toolbar">
            <h3 className="finance-panel__title">
                ...
                <div className="finance-filter-group">
                  {config.statusFilters.map((filterValue) => (
                    <button
                      key={filterValue}
                      type="button"
                      className={`finance-filter-btn ${
                        statusFilter === filterValue ? "finance-filter-btn--active" : ""
                      }`}
                      onClick={() => {
          ...
          {errorMessage && <p className="finance-inline-error">{errorMessage}</p>}
...
export function createFinanceFormPage(config) {
  return function FinanceFormPage() {
    const navigate = useNavigate();
    ...
    return (
      <div className="finance-page finance-crud-page">
        <div className="finance-page__header finance-page__header--with-actions">
          <button
            type="button"
            className="finance-btn finance-btn--back"
        ...
        <form className="finance-form" onSubmit={handleSubmit}>
          <div className="finance-form__fields">
            {config.fields.map((field) => {
              const value = toFieldValue(field, form[field.name]);
              const error = globalErrors[field.name];
              const fieldClassName = [
                "finance-form__field",
                field.fullWidth || field.type === "textarea" ? "finance-form__field--full" : "",
              ]
                .join(" ")
                .trim();
              ...
              return (
                <div key={field.name} className={fieldClassName}>
                  <label htmlFor={field.name} className="finance-form__label">
                    {field.label}
                  </label>
                  ...
                  {field.type === "select" ? (
                    ...
                    <textarea
                      id={field.name}
                      name={field.name}
                      className={`finance-form__input finance-form__textarea ${
                        error ? "finance-form__input--error" : ""
                      }`}
                      value={value}
                      onChange={(event) => handleFieldChange(field.name, event.target.value)}
                      disabled={isSubmitting}
