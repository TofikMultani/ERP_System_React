...
function validateField(field, value) {
  ...
  // Skip validation for readOnly fields (they are auto-populated)
  if (field.readOnly) {
    return "";
  }
...
export function createSalesListPage(config) {
  return function SalesListPage() {
    ...
    return (
      <div className="sales-page sales-crud-page">
        <div className="sales-page__header sales-page__header--with-actions">
          <div>
            <h2>{config.pageTitle}</h2>
            <p>Manage {config.labelPlural.toLowerCase()} with live database records.</p>
          </div>
          <div className="sales-actions">
            <button type="button" className="sales-btn" onClick={() => navigate(config.newRoute)}>
              {config.addLabel}
            </button>
          </div>
        </div>

        <div className="sales-cards sales-cards--compact">
        ...
        <div className="sales-panel">
          <div className="sales-panel__toolbar">
            <h3 className="sales-panel__title">
                  ...
                  {config.statusFilters.map((filterValue) => (
                    <button
                      ...
                      className={`sales-filter-btn ${
                        statusFilter === filterValue ? "sales-filter-btn--active" : ""
                      }`}
                      onClick={() => {
          ...
          {errorMessage && <p className="sales-inline-error">{errorMessage}</p>}
...
export function createSalesFormPage(config) {
  return function SalesFormPage() {
    const navigate = useNavigate();
      ...
      const loadFormData = async () => {
        try {
          if (isEditMode && codeValue) {
            ...
            // For new records, fetch the next auto-generated code
    ...
    function handleFieldChange(fieldName, value) {
      setForm((prevForm) => {
        ...
        // Auto-populate customerName when customerCode changes
        if (fieldName === 'customerCode' && context.customers) {
          const customer = context.customers.find(
            (c) => String(c.code || c.customerCode) === String(value)
          );
          if (customer) {
    ...
    return (
      <div className="sales-page sales-crud-page">
        <div className="sales-page__header sales-page__header--with-actions">
          <button
            type="button"
            className="sales-btn sales-btn--back"
          ...
          <div className="sales-form__fields">
            {config.fields.map((field) => {
              ...
              const fieldClassName = [
                "sales-form__field",
                field.fullWidth || field.type === "textarea" ? "sales-form__field--full" : "",
              ]
                .join(" ")
                .trim();
              ...
              return (
                <div key={field.name} className={fieldClassName}>
                  <label htmlFor={field.name} className="sales-form__label">
                    {field.label}
                  </label>
                  ...
                  {field.type === "select" ? (
                    <select
                      id={field.name}
                      name={field.name}
                      className={`sales-form__input ${error ? "sales-form__input--error" : ""}`}
                      value={value}
                    ...
                    <textarea
                      id={field.name}
                      name={field.name}
                      className={`sales-form__input sales-form__textarea ${
                        error ? "sales-form__input--error" : ""
                      }`}
                      value={value}
                      onChange={(event) => handleFieldChange(field.name, event.target.value)}
                      disabled={isSubmitting}
                    ...
                    <input
                      ...
                      name={field.name}
                      type="checkbox"
                      className="sales-form__checkbox"
                  ...
                  {error && <p className="sales-form__error">{error}</p>}
                </div>
              );
            })}
          </div>
